...
class ScholarshipPeriod(models.Model):
    """One award cycle's completed evaluation period and its outcome.

    The weights/limits used are *snapshotted* from the configuration at
    generation time, so editing the configuration later never silently
    ...
    def __str__(self):
        return f"Стипендия {self.period_start:%d.%m.%Y}–{self.period_end:%d.%m.%Y} (цикл {self.award_day}-го)"
