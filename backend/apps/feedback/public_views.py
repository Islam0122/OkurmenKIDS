...
def _render_message(request, survey: Survey, *, title: str, message: str, icon: str, status=200):
    return render(
        request,
        "feedback/public_message.html",
        {"survey": survey, "title": title, "message": message, "icon": icon},
        status=status,
    )
...
def public_survey_view(request, token):
    ...
    if availability != Survey.Availability.AVAILABLE:
        return _render_message(
            request, survey, title="Опрос недоступен", message=UNAVAILABLE_MESSAGES[availability], icon="lock"
        )
    if not survey.allow_multiple_submissions and has_submitted(request, survey):
        return _render_message(
            request, survey, title="Вы уже ответили", message="Спасибо! Ваш ответ на этот опрос уже получен.", icon="check"
        )
...
def public_survey_done_view(request, token):
    ...
    return _render_message(
        ...
        message=survey.confirmation_message or "Спасибо! Ваш ответ отправлен.",
        icon="check",
    )
