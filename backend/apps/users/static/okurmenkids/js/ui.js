...
(function () {
  ...
  function initTheme() {
    ...
    var loginHost = !nav && document.body.classList.contains("login-page")
      ? document.body
      : null;
    if (!nav && !loginHost) return;

    var btn = document.createElement("button");
    btn.type = "button";
    btn.className = "ok-theme-toggle";
    ...
    btn.addEventListener("click", function () {
      ...
      document.dispatchEvent(
        new CustomEvent("ok-theme-changed", { detail: { theme: next } })
      );
    });

    if (loginHost) {
      loginHost.appendChild(btn);
    } else if (nav.classList.contains("navbar-nav")) {
  ...
  function enhanceMultiSelect(select) {
    ...
    searchBar.innerHTML = '<i class="bi bi-search"></i>';
    ...
    var rows = options.map(function (opt) {
      ...
      row.innerHTML = '<span></span><i class="bi bi-check-lg"></i>';
    ...
    function sync() {
      ...
      rows.forEach(function (item) {
        ...
        if (selected) {
          ...
          removeBtn.setAttribute("aria-label", "Убрать «" + item.opt.text + "»");
      ...
      footer.innerHTML = "Выбрано: <strong>" + selectedCount + "</strong> из " + rows.length;
    }
    ...
    searchInput.addEventListener("input", function () {
      ...
      var visible = 0;
      rows.forEach(function (item) {
        var match = !q || item.row.dataset.label.indexOf(q) !== -1;
  ...
  function initMultiSelects() {
    document
      .querySelectorAll('select[multiple].ok-subject-select, select[multiple].ok-multiselect-source')
  ...
  var SEARCH_HINTS = [
    { match: "/users/teacher/", text: "Поиск тренера по имени, email или телефону..." },
    { match: "/users/subject/", text: "Поиск предмета..." },
    { match: "/users/user/", text: "Поиск администратора..." },
  ];
  ...
  function initSearchHint() {
    ...
    input.placeholder = hint ? hint.text : "Поиск...";
  }
  ...
  var EMPTY_STATES = [
    { match: "/users/teacher/", icon: "bi-person-badge", title: "Пока нет тренеров", text: "Добавьте первого тренера, чтобы начать работу." },
    { match: "/users/subject/", icon: "bi-journal-bookmark", title: "Пока нет предметов", text: "Добавьте предмет, чтобы назначать его тренерам и группам." },
    { match: "/users/user/", icon: "bi-shield-lock", title: "Пока нет администраторов", text: "Добавьте учётную запись администратора." },
  ];

  function initEmptyState() {
    var resultsBox = document.querySelector("#changelist .results");
    var paginator = document.querySelector("#changelist .paginator");
    var hasRows = document.querySelector("#result_list tbody tr");
    if (hasRows || (!resultsBox && !document.getElementById("changelist"))) return;

    // Django renders "0 results" as plain text in the paginator when the
    // table itself is absent (no #result_list at all).
    var changelist = document.getElementById("changelist-form");
    if (!changelist || document.getElementById("result_list")) return;

    var path = window.location.pathname;
    var state = EMPTY_STATES.filter(function (s) {
      return path.indexOf(s.match) !== -1;
    })[0] || { icon: "bi-inbox", title: "Список пуст", text: "Пока здесь нечего показать." };
  ...
  function initActionWarning() {
    ...
    function update() {
      ...
      var isDestructive = /удал|деактив|отмен/i.test(text);
      if (actionsBar) actionsBar.classList.toggle("ok-action-danger", isDestructive);
    }
