...
import { useUpdateMonthlyReportComment } from '@/hooks/useMonthlyReports'
import { extractErrorMessage } from '@/lib/apiError'
import type { MonthlyTeacherReport } from '@/types/monthlyReport'

export function ReportComment({ report, canEdit }: { report: MonthlyTeacherReport; canEdit: boolean }) {
  const [isEditing, setIsEditing] = useState(false)
  const [draft, setDraft] = useState(report.comment)
  const { showToast } = useToast()
  const mutation = useUpdateMonthlyReportComment(report.id)
  ...
  if (isEditing) {
    return (
      <div className="space-y-3">
        <Textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Как прошёл месяц?" autoFocus />
